...
function gettimenow(){
	let date = new Date();
	let now = "" + date.getFullYear() + p(1+date.getMonth()) + p(date.getDate()) + p(date.getHours()) + p(date.getMinutes()) + p(date.getSeconds());
	return now;
}
...
function dumpCPU(handle, agent, comd, context, param, cb) {
	if (handle === 'client') {
		...
		var dumping = agent.get("isCPUDumping");
		if(dumping){
			cb(null, 'cpu dump is doing.');
			return;
		}
		agent.set("isCPUDumping", true);
		profiler.startProfiling('cpu');
		setTimeout(()=>{
			let profile = profiler.stopProfiling();
			let filename = process.cwd() + "/"+ gettimenow()+'.cpuprofile';
			profile.export()
					.pipe(fs.createWriteStream(filename,{flags:'w'}))
					.on('finish', function() {
						profile.delete();
						agent.set("isCPUDumping", false);
						cb(null, agent.id + "=> cpu dump ok:" + filename);
					});
		},times*1000);
...
function dumpMemory(handle, agent, comd, context, param, cb) {
	if (handle === 'client') {
		...
		var filepath = param['filepath'];
		var force = param['force'];
		filepath = process.cwd() + "/"+ gettimenow() + '.heapsnapshot';
		agent.set("isCPUDumping", true);
		try {
			...
			cb(null, agent.id + "=> memory dump ok:" + filepath);
		} catch (e) {
			agent.set("isCPUDumping", false);
			cb(e);
		}
